...
if exists(':FootnoteNumber')
  ...
  " The plugin also auto-maps <Leader>f / <Leader>r. Since <Leader> is <Space>,
  " this makes every Space press in insert mode wait for timeoutlen (500 ms) to
  " see if the next key is f/r, and <Space>f spawns a window. We already use
  " @@ / ^^ for footnotes, so remove the auto-mappings.
